...
void SensorReader::update(ChargeState currentState) {
    DEBUG_F("Charger State: %s", chargeStateToString(currentState));
...
float SensorReader::readCurrent(uint8_t pin) {
    uint32_t mv = analogReadMilliVolts(pin);  // читаем милливольты
    Serial.println(mv);
    float vout = mv / 1000.0f;                // переводим в вольты
    float amp = -(vout - CURRENT_VREF) / CURRENT_KA + 1;
    return amp;
}
